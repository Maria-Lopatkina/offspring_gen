...
def main():
    ...
    for pop in config["populations_all"]:
        ...
        while n != 0:
            father = random.choice(index_list_m)
            mother = random.choice(index_list_f)
                ...
                for k in range(config["kids"]):
                    ...
                    for ii in range(3, len(df_for_lr.index)):
                        ...
                        if "new" in name and "duo" in name:
                            ...
                            offsprings_df.iloc[df_for_lr.index[ii]]["step_mutation"] = steps
                n -= 1
    offsprings_df.to_excel(config["final_output_file_path"], index=True)
