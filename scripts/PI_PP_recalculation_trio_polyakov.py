...
def main():
    ...
    trio_df = pd.read_excel("tuv_v2_trio.xlsx")    # Enter the file name containing trio's data
    ...
    while n != len(output_df):
        for i in range(0, len(trio_df.index), 5):
            real_father_df = trio_df.iloc[[i]]    # real father's data
            ...
            for ch in range(3):
                ...
                for elem in range(1, len(columns_list) - 27, 2):
                    ...
                    rf = [real_father_df.iloc[0][columns_list[elem]], real_father_df.iloc[0][columns_list[elem + 1]]]
                    rm = [real_mother_df.iloc[0][columns_list[elem]], real_mother_df.iloc[0][columns_list[elem + 1]]]
                    rc = [child_df.iloc[0][columns_list[elem]], child_df.iloc[0][columns_list[elem + 1]]]
                    rf.sort()
                    rm.sort()
                    rc.sort()
                    if allele in codis_old:
                        p_old_codis_trio_ref.append(
                            p_calc_trio_wo_mut(rf[0], rf[1], rm[0], rm[1], rc[0], rc[1], ref_dict, allele))
                        p_old_codis_trio_rus.append(
                            p_calc_trio_wo_mut(rf[0], rf[1], rm[0], rm[1], rc[0], rc[1], rus_dict, allele))
                        p_old_codis_duo_ref.append(p_calc_duo_wo_mut(rf[0], rf[1], rc[0], rc[1], ref_dict, allele))
                        p_old_codis_duo_rus.append(p_calc_duo_wo_mut(rf[0], rf[1], rc[0], rc[1], rus_dict, allele))
                    if allele in codis_new:
                        p_new_codis_trio_ref.append(
                            p_calc_trio_wo_mut(rf[0], rf[1], rm[0], rm[1], rc[0], rc[1], ref_dict, allele))
                        p_new_codis_trio_rus.append(
                            p_calc_trio_wo_mut(rf[0], rf[1], rm[0], rm[1], rc[0], rc[1], rus_dict, allele))
                        p_new_codis_duo_ref.append(p_calc_duo_wo_mut(rf[0], rf[1], rc[0], rc[1], ref_dict, allele))
                        p_new_codis_duo_rus.append(p_calc_duo_wo_mut(rf[0], rf[1], rc[0], rc[1], rus_dict, allele))
                    if allele in codis_15:
                        p_codis_15_trio.append(p_calc_trio_wo_mut(rf[0], rf[1], rm[0], rm[1], rc[0], rc[1], ref_dict,
                                                                  allele))
                ...
                for m in p_codis_15_trio:
                    multiplication_15_trio *= m
                output_df.iloc[n]["PI_old_CODIS_trio_ref"] = multiplication_old_trio_ref
                output_df.iloc[n]["PP_old_CODIS_trio_ref"] = multiplication_old_trio_ref / (1 +
                                                                                            multiplication_old_trio_ref)
                output_df.iloc[n]["PI_old_CODIS_duo_ref"] = multiplication_old_duo_ref
                output_df.iloc[n]["PP_old_CODIS_duo_ref"] = multiplication_old_duo_ref / (1 +
                                                                                          multiplication_old_duo_ref)
                output_df.iloc[n]["PI_new_CODIS_trio_ref"] = multiplication_new_trio_ref
                output_df.iloc[n]["PP_new_CODIS_trio_ref"] = multiplication_new_trio_ref / (1 +
                                                                                            multiplication_new_trio_ref)
                output_df.iloc[n]["PI_new_CODIS_duo_ref"] = multiplication_new_duo_ref
                output_df.iloc[n]["PP_new_CODIS_duo_ref"] = multiplication_new_duo_ref / (1 +
                                                                                          multiplication_new_duo_ref)
                output_df.iloc[n]["PI_old_CODIS_trio_rus"] = multiplication_old_trio_rus
                output_df.iloc[n]["PP_old_CODIS_trio_rus"] = multiplication_old_trio_rus / (1 +
                                                                                            multiplication_old_trio_rus)
                output_df.iloc[n]["PI_old_CODIS_duo_rus"] = multiplication_old_duo_rus
                output_df.iloc[n]["PP_old_CODIS_duo_rus"] = multiplication_old_duo_rus / (1 +
                                                                                          multiplication_old_duo_rus)
                output_df.iloc[n]["PI_new_CODIS_trio_rus"] = multiplication_new_trio_rus
                output_df.iloc[n]["PP_new_CODIS_trio_rus"] = multiplication_new_trio_rus / (1 +
                                                                                            multiplication_new_trio_rus)
                output_df.iloc[n]["PI_new_CODIS_duo_rus"] = multiplication_new_duo_rus
                output_df.iloc[n]["PP_new_CODIS_duo_rus"] = multiplication_new_duo_rus / (1 +
                                                                                          multiplication_new_duo_rus)
                output_df.iloc[n]["PI_CODIS_15_trio"] = multiplication_15_trio
                n += 1
    output_df.to_excel("output_PI_PP_trio_tuv_Polyakov.xlsx", index=False)
