...
def trio_freq_father_allele(m1, m2, k1, k2):
    ...
    if k1 != k2:    # Child is heterozygous
        if k1 == m1 and k2 == m2:
            ...
            kn = False
            return f_all, other_all, kn
        if (k1 == m1 and k2 != m2) or (k1 == m2 and k2 != m1) or (k2 == m1 and k1 != m2) or (k2 == m2 and k1 != m1):
            for i in k1, k2:
                for j in m1, m2:
        ...
        for i in range(0, len(trio_df.index), 5):
            ...
            for ch in range(3):
                ...
                for elem in range(1, len(columns_list) - 27, 2):
                    ...
                    f_allele_trio, other_allele_trio, knowledge_trio = trio_freq_father_allele(rm[0], rm[1],
                                                                                               rc[0], rc[1])
                    ...
                    # Choose the dictionary to calculate Q and Q_all for russian ref in trio:
                    ...
                    # Choose the dictionary to calculate Q and Q_all for russian ref in duo:
                ...
                output_df.iloc[n]["PI_old_CODIS_trio_ref"] = 1 / multiplication_old_trio_ref
                output_df.iloc[n]["PP_old_CODIS_trio_ref"] = 1 / (1 + multiplication_old_trio_ref)
                output_df.iloc[n]["PI_old_CODIS_duo_ref"] = 1 / multiplication_old_duo_ref
                output_df.iloc[n]["PP_old_CODIS_duo_ref"] = 1 / (1 + multiplication_old_duo_ref)
                output_df.iloc[n]["PI_new_CODIS_trio_ref"] = 1 / multiplication_new_trio_ref
                output_df.iloc[n]["PP_new_CODIS_trio_ref"] = 1 / (1 + multiplication_new_trio_ref)
                output_df.iloc[n]["PI_new_CODIS_duo_ref"] = 1 / multiplication_new_duo_ref
                output_df.iloc[n]["PP_new_CODIS_duo_ref"] = 1 / (1 + multiplication_new_duo_ref)
                output_df.iloc[n]["PI_old_CODIS_trio_rus"] = 1 / multiplication_old_trio_rus
                output_df.iloc[n]["PP_old_CODIS_trio_rus"] = 1 / (1 + multiplication_old_trio_rus)
                output_df.iloc[n]["PI_old_CODIS_duo_rus"] = 1 / multiplication_old_duo_rus
                output_df.iloc[n]["PP_old_CODIS_duo_rus"] = 1 / (1 + multiplication_old_duo_rus)
                output_df.iloc[n]["PI_new_CODIS_trio_rus"] = 1 / multiplication_new_trio_rus
                output_df.iloc[n]["PP_new_CODIS_trio_rus"] = 1 / (1 + multiplication_new_trio_rus)
                output_df.iloc[n]["PI_new_CODIS_duo_rus"] = 1 / multiplication_new_duo_rus
                output_df.iloc[n]["PP_new_CODIS_duo_rus"] = 1 / (1 + multiplication_new_duo_rus)
                output_df.iloc[n]["PI_CODIS_15_trio"] = 1 / multiplication_15_trio
                n += 1
    output_df.to_excel("YAK_output_PI_PP_trio.xlsx", index=False)
