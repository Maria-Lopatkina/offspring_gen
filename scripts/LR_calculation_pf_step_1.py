...
def main():
    ...
    columns_list.append("LR_ref")
    columns_list.append("LR_rus")
    columns_list.append("step")
        ...
        for ch in range(3):
            ...
            for ii in range(0, len(temp_pf_df.index)):
                ...
                if ("old" in name) and ("trio" in name):
                    for loc in range(1, len(columns_list) - 30, 2):
                        ...
                        if el in codis_old:
                            ...
                            if lack_of_mut:
                                p_1, p_2, p_3, p_4 = hyp_trio_wo_mut(rm[0], rm[1], rc[0], rc[1], our_dict, rus_dict, el)
                            else:
                                steps.append(count_step_trio(pf[0], pf[1], rm[0], rm[1], rc[0], rc[1]))
                                p_1, p_2, p_3, p_4 = hyp_trio_mut(rm[0], rm[1], rc[0], rc[1], our_dict, rus_dict, el,
                                                                  mut_freq)
                ...
                if "old" in name and "duo" in name:
                    for loc in range(1, len(columns_list) - 30, 2):
                        ...
                        if el in codis_old:
                            ...
                            if lack_of_mut:
                                p_1, p_2, p_3, p_4 = hyp_duo_wo_mut(pf[0], pf[1], rc[0], rc[1], our_dict, rus_dict, el)
                            else:
                                steps.append(count_step_duo(pf[0], pf[1], rc[0], rc[1]))
                                p_1, p_2, p_3, p_4 = hyp_duo_mut(rc[0], rc[1], our_dict, rus_dict, el, mut_freq)
                            p_hyp1_list_ref.append(p_1)
                            p_hyp2_list_ref.append(p_2)
                            p_hyp1_list_rus.append(p_3)
                            p_hyp2_list_rus.append(p_4)
                ...
                for m in p_hyp2_list_rus:
                    multiplication_4 *= m
                output_df.iloc[new_index]["LR_ref"] = multiplication_1 / multiplication_2
                output_df.iloc[new_index]["LR_rus"] = multiplication_3 / multiplication_4
                output_df.iloc[new_index]["step"] = steps
                new_index += 1
    output_df.to_excel("output_LR_yak_pf_steps.xlsx", index=False)    # Enter the name of the output file
