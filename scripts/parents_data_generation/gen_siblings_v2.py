...
def main():
    ...
    for pop in config["populations"]:
        ...
        rep1 = copy.deepcopy(config["repeat"])
        rep2 = 0
        while rep1 != rep2:
            list_for_pairs = []
            n = copy.deepcopy(config["number_of_pairs"])
            add_df = pd.DataFrame(columns=columns_list)  # Create additional df
            while n != 0:
                ...
                if pair not in list_for_pairs and father != mother:
                    ...
                    for k in range(config["kids"]):
                        ...
                        kids_df.iloc[k]["№"] = (str(pair_df.iloc[0]['№']) + "-" + str(pair_df.iloc[1]['№']) + "-" +
                                                str(k + 1))
                        for elem in range(1, len(columns_list) - 8, 2):
                            ...
                            m_alleles.sort()
                        if k == 0:
                            for elem in range(len(columns_list) - 7):
                            ...
                            new_pair_df.iloc[0]["Repeat"] = rep2 + 1
                            new_pair_df.iloc[1]["Repeat"] = rep2 + 1
                            ...
                            add_df = pd.concat([add_df, new_pair_df], ignore_index=True)
                        kids_df.iloc[k]["Family_ID"] = (str(pair_df.iloc[0]['№']) + "-" + str(pair_df.iloc[1]['№']))
                        kids_df.iloc[k]["population"] = pop
                        kids_df.iloc[k]["Repeat"] = rep2 + 1
                        add_df = pd.concat([add_df, kids_df.loc[[k]]], ignore_index=True)
                n -= 1

                #######################
                # Count LR
                df_for_lr = add_df.loc[(add_df['Status'] == "kid")]
                proband_df = df_for_lr.iloc[[0]]
                for ii in range(1, len(df_for_lr.index)):
                    hyp1_ref_old = []
                    hyp2_ref_old = []
                    hyp1_rus_old = []
                    hyp2_rus_old = []
                    ...
                    for loc in range(1, len(columns_list) - 8, 2):
                    ...
                    for m in hyp2_rus_new:
                        multiplication_8 *= m
                    add_df.iloc[df_for_lr.index[ii]]["LR_ref_old_codis"] = multiplication_1 / multiplication_2
                    add_df.iloc[df_for_lr.index[ii]]["LR_rus_old_codis"] = multiplication_3 / multiplication_4
                    add_df.iloc[df_for_lr.index[ii]]["LR_ref_new_codis"] = multiplication_5 / multiplication_6
                    add_df.iloc[df_for_lr.index[ii]]["LR_rus_new_codis"] = multiplication_7 / multiplication_8
            offsprings_df = pd.concat([offsprings_df, add_df], ignore_index=True)
            rep2 += 1
    offsprings_df.to_excel("siblings_new_TEST.xlsx", index=True)
