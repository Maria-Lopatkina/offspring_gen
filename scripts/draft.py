"""
Script for PI, PP recalculation for potential fathers. It should be used if the table with new population
is already generated. This script uses PI and PP formulas taken from the site of the Center of Molecular Genetics:
...
def equations_mut(a1, a2, n, dic, allele, mut):
...
def p_calc_trio_mut(f1, f2, m1, m2, k1, k2, dic, allele, mut):
    """
    If we know the maternal allele:
        if f1 == f2:
            p = m / (4 * P_f1) / 10 ** (step - 1)
        if f1 != f2:
            p = m / (2 * P_f1) / 10 ** (step_1 - 1) / 10 ** (step_2 - 1)
    elif we don't know the maternal allele:
        if 1 single step mut:
            p = 1 / (4 * (P_k1 + P_k2))
        if 3 single step mut:
            p = 3 / (4 * (P_k1 + P_k2))
        if 2:
            p = ??????
        ...
        for i in range(0, len(trio_df.index), 5):
            ...
            for ch in range(3):
                ...
                for ii in range(0, len(temp_pf_df.index)):
                    ...
                    p_old_codis_trio_ref = []
                    p_new_codis_trio_ref = []
                    p_old_codis_duo_ref = []
                    p_new_codis_duo_ref = []
                    p_old_codis_trio_rus = []
                    p_new_codis_trio_rus = []
                    p_old_codis_duo_rus = []
                    p_new_codis_duo_rus = []
                    # OLD CODIS and TRIO
                    if ("old" in name) and ("trio" in name):
                        for loc in range(1, len(columns_list) - 27, 2):
                            ...
                            if el in codis_old:
                                ...
                                if lack_of_mut:
                                    p_old_codis_trio_ref.append(
                                        p_calc_trio_wo_mut(pf[0], pf[1], rm[0], rm[1], rc[0], rc[1], ref_dict, el))
                                    p_old_codis_trio_rus.append(
                                        p_calc_trio_wo_mut(pf[0], pf[1], rm[0], rm[1], rc[0], rc[1], rus_dict, el))
                                else:
                                    p_old_codis_trio_ref.append(p_calc_trio_mut(pf[0], pf[1], rm[0], rm[1], rc[0],
                                                                                rc[1], ref_dict, el, mut_freq))
                                    p_old_codis_trio_rus.append(p_calc_trio_mut(pf[0], pf[1], rm[0], rm[1], rc[0],
                                                                                rc[1], rus_dict, el, mut_freq))
                        multiplication_old_trio_ref = 1
                        multiplication_old_trio_rus = 1
                        for m in p_old_codis_trio_ref:
                            multiplication_old_trio_ref *= m
                        for m in p_old_codis_trio_rus:
                            multiplication_old_trio_rus *= m
                        output_df.iloc[n]["p_father_PI_ref"] = multiplication_old_trio_ref
                        output_df.iloc[n]["p_father_PP_ref"] = multiplication_old_trio_ref / \
                                                               (1 + multiplication_old_trio_ref)
                        output_df.iloc[n]["p_father_PI_rus"] = multiplication_old_trio_rus
                        output_df.iloc[n]["p_father_PP_rus"] = multiplication_old_trio_rus / \
                                                               (1 + multiplication_old_trio_rus)
                    ...
                    if "old" in name and "duo" in name:
                        for loc in range(1, len(columns_list) - 27, 2):
                            ...
                            if el in codis_old:
                                ...
                                if lack_of_mut:
                                    p_old_codis_duo_ref.append(p_calc_duo_wo_mut(pf[0], pf[1], rc[0], rc[1],
                                                                                 ref_dict, el))
                                    p_old_codis_duo_rus.append(p_calc_duo_wo_mut(pf[0], pf[1], rc[0], rc[1],
                                                                                 rus_dict, el))
                                else:
                                    p_old_codis_duo_ref.append(p_calc_duo_mut(pf[0], pf[1], rc[0], rc[1], ref_dict, el,
                                                                              mut_freq))
                                    p_old_codis_duo_rus.append(p_calc_duo_mut(pf[0], pf[1], rc[0], rc[1], rus_dict, el,
                                                                              mut_freq))
                        multiplication_old_duo_ref = 1
                        multiplication_old_duo_rus = 1
                        for m in p_old_codis_duo_ref:
                            multiplication_old_duo_ref *= m
                        for m in p_old_codis_duo_rus:
                            multiplication_old_duo_rus *= m
                        output_df.iloc[n]["p_father_PI_ref"] = multiplication_old_duo_ref
                        output_df.iloc[n]["p_father_PP_ref"] = multiplication_old_duo_ref / \
                                                               (1 + multiplication_old_duo_ref)
                        output_df.iloc[n]["p_father_PI_rus"] = multiplication_old_duo_rus
                        output_df.iloc[n]["p_father_PP_rus"] = multiplication_old_duo_rus / \
                                                               (1 + multiplication_old_duo_rus)
                        output_df.iloc[n]["Child_ID"] = child_df.iloc[0]["№"]
                        n += 1
                    # NEW CODIS and TRIO
                    if "new" in name and "trio" in name:
                        for loc in range(1, len(columns_list) - 27, 2):
                            ...
                            if el in codis_new:
                                ...
                                lack_of_mut = father_trio(pf[0], pf[1], rm[0], rm[1], rc[0], rc[1])
                                if lack_of_mut:
                                    p_new_codis_trio_ref.append(
                                        p_calc_trio_wo_mut(pf[0], pf[1], rm[0], rm[1], rc[0], rc[1], ref_dict, el))
                                    p_new_codis_trio_rus.append(
                                        p_calc_trio_wo_mut(pf[0], pf[1], rm[0], rm[1], rc[0], rc[1], rus_dict, el))
                                else:
                                    p_new_codis_trio_ref.append(p_calc_trio_mut(pf[0], pf[1], rm[0], rm[1], rc[0],
                                                                                rc[1], ref_dict, el, mut_freq))
                                    p_new_codis_trio_rus.append(p_calc_trio_mut(pf[0], pf[1], rm[0], rm[1], rc[0],
                                                                                rc[1], rus_dict, el, mut_freq))
                        multiplication_new_trio_ref = 1
                        multiplication_new_trio_rus = 1
                        for m in p_new_codis_trio_ref:
                            multiplication_new_trio_ref *= m
                        for m in p_new_codis_trio_rus:
                            multiplication_new_trio_rus *= m
                        output_df.iloc[n]["p_father_PI_ref"] = multiplication_new_trio_ref
                        output_df.iloc[n]["p_father_PP_ref"] = multiplication_new_trio_ref / \
                                                               (1 + multiplication_new_trio_ref)
                        output_df.iloc[n]["p_father_PI_rus"] = multiplication_new_trio_rus
                        output_df.iloc[n]["p_father_PP_rus"] = multiplication_new_trio_rus / \
                                                               (1 + multiplication_new_trio_rus)
                        output_df.iloc[n]["Child_ID"] = child_df.iloc[0]["№"]
                        n += 1
                    # NEW CODIS and DUO
                    if "new" in name and "duo" in name:
                        for loc in range(1, len(columns_list) - 27, 2):
                            ...
                            if el in codis_new:
                                ...
                                lack_of_mut = father_duo(pf[0], pf[1], rc[0], rc[1])
                                if lack_of_mut:
                                    p_new_codis_duo_ref.append(p_calc_duo_wo_mut(pf[0], pf[1], rc[0], rc[1],
                                                                                 ref_dict, el))
                                    p_new_codis_duo_rus.append(p_calc_duo_wo_mut(pf[0], pf[1], rc[0], rc[1],
                                                                                 rus_dict, el))
                                else:
                                    p_new_codis_duo_ref.append(p_calc_duo_mut(pf[0], pf[1], rc[0], rc[1], ref_dict, el,
                                                                              mut_freq))
                                    p_new_codis_duo_rus.append(p_calc_duo_mut(pf[0], pf[1], rc[0], rc[1], rus_dict, el,
                                                                              mut_freq))
                        multiplication_new_duo_ref = 1
                        multiplication_new_duo_rus = 1
                        for m in p_new_codis_duo_ref:
                            multiplication_new_duo_ref *= m
                        for m in p_new_codis_duo_rus:
                            multiplication_new_duo_rus *= m
                        output_df.iloc[n]["p_father_PI_ref"] = multiplication_new_duo_ref
                        output_df.iloc[n]["p_father_PP_ref"] = multiplication_new_duo_ref / \
                                                               (1 + multiplication_new_duo_ref)
                        output_df.iloc[n]["p_father_PI_rus"] = multiplication_new_duo_rus
                        output_df.iloc[n]["p_father_PP_rus"] = multiplication_new_duo_rus / \
                                                               (1 + multiplication_new_duo_rus)
                        output_df.iloc[n]["Child_ID"] = child_df.iloc[0]["№"]
                        n += 1
