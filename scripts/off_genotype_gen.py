...
def calculate_frequencies(p, d):  # Calculate alleles frequencies for three populations
    freq_df = pd.read_excel("ASTR22_main.xlsx")
    f_columns = freq_df.columns.values.tolist()
    temp_freq_df = freq_df.query("population == @p")
    for al in range(1, len(f_columns) - 2, 2):
        ...
        aa = a.to_dict()
        keyslist = list(aa.keys())
        newkeyslist = []
        for h in keyslist:
            newkeyslist.append(h[0])
        new_aa = d.fromkeys(newkeyslist)
        for h in range(0, len(newkeyslist)):
            new_aa[newkeyslist[h]] = aa[keyslist[h]]
...
def trio_freq_father_allele(f1, f2, m1, m2, k1, k2):
    f_all = None
    other_all = None
    if k1 == k2 or f1 == f2:    # Father or child is homozygote
        if k1 == k2:
            f_all = k1
        elif f1 == f2:
            f_all = f1
    ...
    if (f1 != f2) and (m1 != m2) and (f1 != m1) and (f1 != m2) and (f2 != m1) and (f2 != m2):
        # Father and mother are heterozygote, w/o common alleles
        for i in f1, f2:
            for j in k1, k2:
                if i == j:
                    f_all = i
                    kn = True
                    return f_all, other_all, kn
    if m1 == m2:    # Mother is homozygote, father and child are heterozygote:
        if k1 == m1:
            f_all = k2
        elif k2 == m1:
            f_all = k1
        kn = True
        return f_all, other_all, kn
    if f1 == m1 == k1 and f2 == m2 == k2:    # Trio are heterozygote with two common alleles
        f_all = k1
        other_all = k2
        kn = False
        return f_all, other_all, kn
    if f1 == k1 and f2 == k2:    # Father and child are heterozygote with two common alleles
        if m1 == k1 or m2 == k1:
            f_all = k2
        elif m1 == k2 or m2 == k2:
            f_all = k1
        kn = True
        return f_all, other_all, kn
    if m1 == k1 and m2 == k2:    # Mother and child are heterozygote with two common alleles
        if f1 == k1 or f2 == k1:
            f_all = k1
        elif f1 == k2 or f2 == k2:
            f_all = k2
        kn = True
        return f_all, other_all, kn
    else:    # In a case of f 13/14 m 14/15 c 13/15
        for i in f1, f2:
            for j in k1, k2:
                if i == j:
                    f_all = i
...
def duo_freq_father_allele(f1, f2, k1, k2):
    f_all = None
    other_all = None
    if k1 == k2 or f1 == f2:    # Father or child is homozygote
        if k1 == k2:
        ...
        kn = True
        return f_all, other_all, kn
    if f1 == k1 and f2 == k2:    # Duo are heterozygote with two common alleles
        f_all = k1
        other_all = k2
        kn = False
        return f_all, other_all, kn
...
def calculate_q(str_list, allele_name, population, gin_dict, rus_dict, yak_dict, f_a, o_a, know):
    q = None
    if allele_name in str_list:
        if know:
            if population == "гинухцы":
                q = gin_dict[allele_name][f_a] * (2 - gin_dict[allele_name][f_a])
            if population == "русские":
                q = rus_dict[allele_name][f_a] * (2 - rus_dict[allele_name][f_a])
            if population == "якуты":
                q = yak_dict[allele_name][f_a] * (2 - yak_dict[allele_name][f_a])
        else:
            if population == "гинухцы":
                q = (gin_dict[allele_name][f_a] + gin_dict[allele_name][o_a]) * (2 - (gin_dict[allele_name][f_a]
                                                                                      + gin_dict[allele_name][o_a]))
            if population == "русские":
                q = (rus_dict[allele_name][f_a] + rus_dict[allele_name][o_a]) * (2 - (rus_dict[allele_name][f_a]
                                                                                      + rus_dict[allele_name][o_a]))
            if population == "якуты":
                q = (yak_dict[allele_name][f_a] + yak_dict[allele_name][o_a]) * (2 - (yak_dict[allele_name][f_a]
                                                                                      + yak_dict[allele_name][o_a]))
        return q
    else:
        return None


def calculate_q_ref(str_list, allele_name, r_dict, f_a, o_a, know):
    q = None
    if allele_name in str_list:
        if know:
            if f_a in r_dict[allele_name]:
                q = r_dict[allele_name][f_a] * (2 - r_dict[allele_name][f_a])
            else:
                q = r_dict[allele_name]["pmin"] * (2 - r_dict[allele_name]["pmin"])
        else:
            if f_a in r_dict[allele_name] and o_a in r_dict[allele_name]:
                q = (r_dict[allele_name][f_a] + r_dict[allele_name][o_a]) * (2 - (r_dict[allele_name][f_a]
                                                                                  + r_dict[allele_name][o_a]))
            elif f_a not in r_dict[allele_name] and o_a in r_dict[allele_name]:
                q = (r_dict[allele_name]["pmin"] + r_dict[allele_name][o_a]) * (2 - (r_dict[allele_name]["pmin"]
                                                                                     + r_dict[allele_name][o_a]))
            elif f_a in r_dict[allele_name] and o_a not in r_dict[allele_name]:
                q = (r_dict[allele_name][f_a] + r_dict[allele_name]["pmin"]) * (2 - (r_dict[allele_name][f_a]
                                                                                     + r_dict[allele_name]["pmin"]))
            elif f_a not in r_dict[allele_name] and o_a not in r_dict[allele_name]:
                q = (r_dict[allele_name]["pmin"] + r_dict[allele_name]["pmin"]) * (2 - (r_dict[allele_name]["pmin"]
                                                                                        + r_dict[allele_name]["pmin"]))
        return q
    else:
        return None
...
def main():
    ...
    gin = empty_freq_table()
    rus = empty_freq_table()
    yak = empty_freq_table()
    gin = calculate_frequencies("гинухцы", gin)
    rus = calculate_frequencies("русские", rus)
    yak = calculate_frequencies("якуты", yak)
    ref_dict = {"CSF1PO": {7: 0.0004, 8: 0.0012, 9: 0.0438, 10: 0.2824, 11: 0.2882, 12: 0.3062, 13: 0.0617, 14: 0.0127,
    ...
    populations = ["гинухцы", "русские", "якуты"]
    ...
    for pop in populations:
        temp_parents_df = parents_df.query("population == @pop")
            ...
            if pair not in list_for_pairs and father != mother:
                ...
                for k in range(kids):
                    ...
                    for elem in range(1, len(columns_list) - 27, 2):
                        ...
                        f_allele_trio, other_allele_trio, knowledge_trio = trio_freq_father_allele(f_alleles[0],
                                                                                                   f_alleles[1],
                                                                                                   m_alleles[0],
                                                                                                   m_alleles[1],
                                                                                                   kid_alleles[0],
                        ...
                        q_old_trio_ref = calculate_q(codis_old, allele, pop, gin, rus, yak, f_allele_trio,
                                                     other_allele_trio, knowledge_trio)
                        if q_old_trio_ref:
                            q_old_codis_trio_ref.append(q_old_trio_ref)
                        q_new_trio_ref = calculate_q(codis_new, allele, pop, gin, rus, yak, f_allele_trio,
                                                     other_allele_trio, knowledge_trio)
                        if q_new_trio_ref:
                            q_new_codis_trio_ref.append(q_new_trio_ref)
                        q_trio_15 = calculate_q(codis_15, allele, pop, gin, rus, yak, f_allele_trio, other_allele_trio,
                                                knowledge_trio)
                        if q_trio_15:
                            q_codis_15_trio.append(q_trio_15)
                        # Check the father allele in a case of duo
                        f_allele_duo, other_allele_duo, knowledge_duo = duo_freq_father_allele(f_alleles[0],
                                                                                               f_alleles[1],
                                                                                               kid_alleles[0],
                                                                                               kid_alleles[1])
                        # Choose the dictionary to calculate Q and Q_all:
                        q_old_duo_ref = calculate_q(codis_old, allele, pop, gin, rus, yak, f_allele_duo,
                                                    other_allele_duo, knowledge_duo)
                        if q_old_duo_ref:
                            q_old_codis_duo_ref.append(q_old_duo_ref)
                        q_new_duo_ref = calculate_q(codis_new, allele, pop, gin, rus, yak, f_allele_duo,
                                                    other_allele_duo, knowledge_duo)
                        if q_new_duo_ref:
                            q_new_codis_duo_ref.append(q_new_duo_ref)
                        # Choose the dictionary to calculate Q and Q_all for new ref in trio:
                        q_old_trio_rus = calculate_q_ref(codis_old, allele, ref_dict, f_allele_trio, other_allele_trio,
                                                         knowledge_trio)
                        if q_old_trio_rus:
                            q_old_codis_trio_rus.append(q_old_trio_rus)
                        q_new_trio_rus = calculate_q_ref(codis_new, allele, ref_dict, f_allele_trio, other_allele_trio,
                                                         knowledge_trio)
                        if q_new_trio_rus:
                            q_new_codis_trio_rus.append(q_new_trio_rus)
                        # Choose the dictionary to calculate Q and Q_all for new ref in duo:
                        q_old_duo_rus = calculate_q_ref(codis_old, allele, ref_dict, f_allele_duo, other_allele_duo,
                                                        knowledge_duo)
                        if q_old_duo_rus:
                            q_old_codis_duo_rus.append(q_old_duo_rus)
                        q_new_duo_rus = calculate_q_ref(codis_new, allele, ref_dict, f_allele_duo, other_allele_duo,
                                                        knowledge_duo)
                        if q_new_duo_rus:
                    ...
                    for ii in ids1:
                        ...
                        for elem in range(1, len(columns_list) - 27, 2):
                            ...
                            if (allele not in ids1[ii]) and (allele in codis_old):
                                f_alleles = [p_pair_df.iloc[0][columns_list[elem]],
                                             p_pair_df.iloc[0][columns_list[elem + 1]]]
                                ...
                                q_old_trio_ref = calculate_q(codis_old, allele, pop, gin, rus, yak, f_allele_trio,
                                                             other_allele_trio, knowledge_trio)
                                q_old_trio_rus = calculate_q_ref(codis_old, allele, ref_dict, f_allele_trio,
                                                                 other_allele_trio, knowledge_trio)
                                if q_old_trio_ref:
                                    q_old_codis_trio_ref.append(q_old_trio_ref)
                                if q_old_trio_rus:
                    ...
                    for ii in ids2:
                        ...
                        for elem in range(1, len(columns_list) - 27, 2):
                            ...
                            if (allele not in ids2[ii]) and (allele in codis_old):
                                ...
                                q_old_duo_ref = calculate_q(codis_old, allele, pop, gin, rus, yak, f_allele_duo,
                                                            other_allele_duo, knowledge_duo)
                                q_old_duo_rus = calculate_q_ref(codis_old, allele, ref_dict, f_allele_duo,
                                                                other_allele_duo, knowledge_duo)
                                if q_old_duo_ref:
                                    q_old_codis_duo_ref.append(q_old_duo_ref)
                                if q_old_duo_rus:
                    ...
                    for ii in ids3:
                        ...
                        for elem in range(1, len(columns_list) - 27, 2):
                            ...
                            if (allele not in ids3[ii]) and (allele in codis_new):
                                ...
                                q_new_trio_ref = calculate_q(codis_new, allele, pop, gin, rus, yak, f_allele_trio,
                                                             other_allele_trio, knowledge_trio)
                                q_new_trio_rus = calculate_q_ref(codis_new, allele, ref_dict, f_allele_trio,
                                                                 other_allele_trio, knowledge_trio)
                                if q_new_trio_ref:
                                    q_new_codis_trio_ref.append(q_new_trio_ref)
                                if q_new_trio_rus:
                    ...
                    for ii in ids4:
                        ...
                        for elem in range(1, len(columns_list) - 27, 2):
                            ...
                            if (allele not in ids4[ii]) and (allele in codis_new):
                                ...
                                q_new_duo_ref = calculate_q(codis_new, allele, pop, gin, rus, yak, f_allele_duo,
                                                            other_allele_duo, knowledge_duo)
                                q_new_duo_rus = calculate_q_ref(codis_new, allele, ref_dict, f_allele_duo,
                                                                other_allele_duo, knowledge_duo)
                                if q_new_duo_ref:
                                    q_new_codis_duo_ref.append(q_new_duo_ref)
                                if q_new_duo_rus:
    ...
    offsprings_df.to_excel("output.xlsx", index=False)
